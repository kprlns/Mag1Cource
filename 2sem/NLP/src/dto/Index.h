...
class Index {
public:
    HashSet<Pair<unsigned long long, Vector<int>*>*>* index;
    Vector<pos_type>* docPositions;


    Index() {
        index = new HashSet<Pair<unsigned long long, Vector<int>*>*>(100000);
    ...
    void putAll(HashSet<unsigned long long>* allHashes, int docId, pos_type docPosition) {
        for(int i = 0; allHashes->getSize(); ++i) {
    ...
    void putOne(unsigned long long hash, int docId) {
        ...
        auto setItem = index->put(newPair);
